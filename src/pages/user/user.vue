	...
	<div class="user">
		...
		<div v-else>
			<div class="user-h islogin">
				...
				<div class="user-d">
					<p class="username">{{userState.username}}</p>
		...
		<div class="user-h-list">
			<ul>
				<li>
					<router-link to="">
						<span class="num" v-show="isLogin">{{pubTotravelNum + pubTosportNum}}</span>
						<span>发布</span>
					</router-link>
				</li>
				<li>
					<router-link to="">
						<span class="num" v-show="isLogin">{{pubTosportNum}}</span>
						<span>约跑</span>
					</router-link>
				</li>
				<li>
					<router-link to="">
						<span class="num" v-show="isLogin">{{pubTotravelNum}}</span>
						<span>约行</span>
					</router-link>
				</li>
			</ul>
...
export default {
	...
	created () {
		...
		if (userId && phone) {
			this.isLogin = true
			post('/users/api/getPubTotravelNum', {userId: userId})
				.then(response => {
					this.pubTotravelNum = response.data.data
				})
		} else {
			this.isLogin = false
		}
